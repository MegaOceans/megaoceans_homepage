...
const SvgBG = () => {
  ...
  useEffect(() => {
    ...
    const scrolleffect = document.querySelectorAll(".wave");
    scrolleffect.forEach((wave, index) => {
      wave.style.transform = `translateY(${scrollYa + index * 10}px)`;
      // }).style.transform = "scale(1 + scrollY, 1 + scrollY)";
    });
  }, [scrollYa]);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      console.log("svasagsfgf" + window.scrollY);
  ...
  return (
    <svg id="visual" viewBox="0 0 900 600" width="900" height="600">
      ...
      <g className="wave" style={{ transform: "translateX(0)" }}>
        <path
      ...
      <g className="wave" style={{ transform: "scale(1.2,1.2)" }}>
        <path
